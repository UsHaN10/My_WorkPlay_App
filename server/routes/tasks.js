        ...
        const tasks = await Task.findAll({
            where: {
                [Sequelize.Op.or]: [
                    ...
                    { assignedToUserId: null, targetRole: fullUser.jobRole || null },
                    { assignedToUserId: null, targetRole: null } // Global tasks
                ]
            }
        });
        res.json(tasks);
...
router.post('/:taskId/submit', authenticateToken, upload.single('verificationPhoto'), async (req, res) => {
    try {
        ...
        // Assign task to submittor if it's a global task
